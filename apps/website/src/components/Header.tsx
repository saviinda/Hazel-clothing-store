...
function NavLink({
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`whitespace-nowrap transition-colors duration-300 hover:text-brand-primary ${
        active ? 'text-brand-primary font-bold border-b border-brand-primary/45 pb-1' : 'border-b border-transparent pb-1'
      }`}
    >
      {label}
    </Link>
      ...
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 md:px-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex flex-col leading-tight">
            <span className="font-serif text-lg font-bold tracking-wide text-brand-secondary">HAZEL</span>
            <span className="text-[8px] tracking-[0.22em] text-brand-secondary/50 uppercase">Clothing Boutique</span>
          </div>
        </div>

        {/* Center: Desktop Navigation */}
        <nav className="hidden lg:flex items-center gap-6 xl:gap-8 text-[11px] font-medium tracking-[0.2em] text-brand-secondary">
          {NAV_LINKS.map((link) => (
            <NavLink
              key={link.href}
              href={link.href}
